...
function buildDailyRecommendations() {
  ...
  for (const game of state.games) {
    ...
    const gameSignals = createGameMarketSignals(game, odds);
    candidates.push(...gameSignals);
  }

  return candidates
    .filter(item => item.confidence >= 58 && item.ev != null)
    .sort((a, b) => {
...
function createGameMarketSignals(game, odds) {
  const signals = [];
  const ml = odds?.moneyline || {};

  if (isValidAmericanOdds(ml.away) && isValidAmericanOdds(ml.home)) {
    const awayImplied = americanImpliedProbability(ml.away);
    const homeImplied = americanImpliedProbability(ml.home);
    const normalized = normalizeTwoWayProbabilities(awayImplied, homeImplied);

    const awayModel = estimateTeamProbability(game, "away", normalized.away);
    const homeModel = estimateTeamProbability(game, "home", normalized.home);

    signals.push(makeRecommendation({
      game,
      market: "Moneyline",
      selection: `${game.away} ML`,
      american: ml.away,
      modelProbability: awayModel.probability,
      confidence: awayModel.confidence,
      reasons: awayModel.reasons,
      side: "away",
    }));
    ...
    signals.push(makeRecommendation({
      game,
      market: "Moneyline",
      selection: `${game.home} ML`,
      american: ml.home,
      modelProbability: homeModel.probability,
      confidence: homeModel.confidence,
      reasons: homeModel.reasons,
      side: "home",
    }));
  }

  const total = odds?.total;
  if (total?.point != null && isValidAmericanOdds(total.over) && isValidAmericanOdds(total.under)) {
    const totalSignal = estimateTotalProbability(game, total);
    signals.push(makeRecommendation({
      game,
      market: "Total",
      selection: `Over ${total.point}`,
      american: total.over,
      modelProbability: totalSignal.overProbability,
      confidence: totalSignal.overConfidence,
      reasons: totalSignal.overReasons,
      side: "over",
    }));
    signals.push(makeRecommendation({
      game,
      ...
      american: total.under,
      modelProbability: totalSignal.underProbability,
      confidence: totalSignal.underConfidence,
      reasons: totalSignal.underReasons,
      side: "under",
    }));
  }
...
function makeRecommendation({ game, market, selection, american, modelProbability, confidence, reasons, side }) {
  const implied = americanImpliedProbability(american);
  const decimal = americanToDecimal(american);
  const ev = Number.isFinite(modelProbability) && Number.isFinite(decimal)
    ? modelProbability * decimal - 1
    : null;

  return {
    id: `${game.id}-${market}-${selection}`,
    game,
    market,
    selection,
    american: Number(american),
    ...
    reasons,
    side,
    grade: recommendationGrade(confidence, ev),
  };
}

function recommendationGrade(confidence, ev) {
  if (confidence >= 78 && ev >= 0.06) return "A";
  if (confidence >= 70 && ev >= 0.03) return "B+";
  if (confidence >= 64 && ev >= 0.01) return "B";
  if (confidence >= 58) return "C+";
  return "Evitar";
}

function normalizeTwoWayProbabilities(away, home) {
  const total = away + home || 1;
  return { away: away / total, home: home / total };
}

function estimateTeamProbability(game, side, marketBaseline) {
  let adjustment = 0;
  let evidence = 0;
  const reasons = [];

  const ownRecord = parseRecord(side === "away" ? game.awayRecord : game.homeRecord);
  const oppRecord = parseRecord(side === "away" ? game.homeRecord : game.awayRecord);
  if (ownRecord && oppRecord) {
    evidence += 1;
    const recordEdge = ownRecord.pct - oppRecord.pct;
    adjustment += clamp(recordEdge * 0.18, -0.045, 0.045);
    if (Math.abs(recordEdge) >= 0.04) {
      reasons.push(recordEdge > 0 ? "Mejor porcentaje de victorias" : "Peor porcentaje de victorias");
    }
  }

  const isHome = side === "home";
  adjustment += isHome ? 0.018 : -0.008;
  evidence += 0.5;
  reasons.push(isHome ? "Ventaja de localía" : "Condición de visitante");

  const ownPitcher = side === "away" ? game.awayPitcher : game.homePitcher;
  const oppPitcher = side === "away" ? game.homePitcher : game.awayPitcher;
  if (ownPitcher && ownPitcher !== "Por confirmar") {
    evidence += 0.5;
    reasons.push("Abridor confirmado");
  }
  if (!oppPitcher || oppPitcher === "Por confirmar") {
    adjustment += 0.01;
  }

  const weather = game.weather || {};
  const wind = parseStatNumber(weather.wind_speed_10m);
  if (wind != null && wind >= 22) {
    evidence += 0.25;
    reasons.push("Viento elevado aumenta volatilidad");
  }

  const probability = clamp(marketBaseline + adjustment, 0.12, 0.88);
  const distance = Math.abs(probability - 0.5);
  const confidence = clamp(52 + distance * 110 + evidence * 3.5, 45, 86);
...
function estimateTotalProbability(game, total) {
  let over = 0.5;
  let evidence = 0;
  const overReasons = [];
  const underReasons = [];

  const point = Number(total.point);
  if (Number.isFinite(point)) {
    if (point <= 7.5) {
      over += 0.035;
      overReasons.push("Total de mercado relativamente bajo");
    } else if (point >= 9.5) {
      over -= 0.035;
      underReasons.push("Total de mercado relativamente alto");
    }
  }

  const weather = game.weather || {};
  const temp = parseStatNumber(weather.temperature_2m);
  const wind = parseStatNumber(weather.wind_speed_10m);
  const humidity = parseStatNumber(weather.relative_humidity_2m);

  if (temp != null) {
    evidence += 0.5;
    if (temp >= 28) {
      over += 0.025;
      overReasons.push("Temperatura alta");
    } else if (temp <= 15) {
      over -= 0.025;
      underReasons.push("Temperatura baja");
    }
  }
  ...
  if (wind != null) {
    ...
    if (wind >= 20) {
      over += 0.02;
      overReasons.push("Viento fuerte");
    }
  }
  ...
  if (humidity != null && humidity >= 80) {
    evidence += 0.25;
    over += 0.008;
    overReasons.push("Humedad elevada");
  }

  const bothConfirmed = game.awayPitcherId && game.homePitcherId;
  if (bothConfirmed) {
    evidence += 0.5;
    underReasons.push("Ambos abridores confirmados");
  }

  over = clamp(over, 0.25, 0.75);
  const under = 1 - over;
  const overConfidence = clamp(50 + Math.abs(over - 0.5) * 125 + evidence * 3, 48, 84);
  const underConfidence = clamp(50 + Math.abs(under - 0.5) * 125 + evidence * 3, 48, 84);
  ...
  return {
    ...
    overConfidence,
    underConfidence,
    overReasons: overReasons.length ? overReasons : ["Señales mixtas para el Over"],
    underReasons: underReasons.length ? underReasons : ["Señales mixtas para el Under"],
  };
}
...
function parseRecord(record) {
  ...
  return { wins, losses, pct: wins / total };
}

function renderRecommendationsDashboard() {
  const existing = document.querySelector("#recommendationsSection");
  if (existing) existing.remove();

  const gamesSection = document.querySelector("#gamesSection") || document.querySelector(".games-section");
  ...
  section.innerHTML = `
    <div class="section-head">
      <div>
        <p class="eyebrow">ANÁLISIS AUTOMÁTICO</p>
        <h2>Oportunidades del día</h2>
      </div>
      <span class="muted">${recommendations.length} señales</span>
    </div>
    <div class="recommendation-disclaimer">
      Confianza y EV son estimaciones heurísticas, no probabilidades validadas ni garantía de ganancia.
    </div>
    <div class="recommendations-grid">
      ${recommendations.length
        ? recommendations.map(renderRecommendationCard).join("")
        : '<div class="card empty-recommendations">No hay señales que superen el filtro mínimo.</div>'}
    </div>`;

  gamesSection.parentNode.insertBefore(section, gamesSection);

  section.querySelectorAll("[data-add-recommendation]").forEach(button => {
    button.addEventListener("click", () => {
      const recommendation = recommendations.find(item => item.id === button.dataset.addRecommendation);
      if (!recommendation) return;
      try {
        addPickToParley({
          game: recommendation.game,
          selection: recommendation.selection,
          american: recommendation.american,
          source: "ParleyLab 5.0",
          market: recommendation.market,
        });
        showToast("Pick agregado al boleto.");
      } catch (error) {
        showToast(error.message || "No se pudo agregar el pick.", "error");
      }
    });
  });
}
  ...
  return `
    <article class="recommendation-card card">
      <div class="recommendation-top">
        <span class="grade grade-${item.grade.replace("+", "plus").toLowerCase()}">${item.grade}</span>
        <span>${item.market}</span>
      </div>
      <h3>${escapeHtml(item.selection)}</h3>
      <p class="muted">${escapeHtml(item.game.away)} @ ${escapeHtml(item.game.home)}</p>
      <div class="recommendation-metrics">
        <div><span>Confianza</span><strong>${item.confidence}%</strong></div>
        <div><span>Modelo</span><strong>${(item.modelProbability * 100).toFixed(1)}%</strong></div>
        <div><span>Implícita</span><strong>${(item.implied * 100).toFixed(1)}%</strong></div>
        <div><span>EV estimado</span><strong class="${item.ev >= 0 ? "positive" : "negative"}">${evText}</strong></div>
      </div>
      <ul class="recommendation-reasons">
        ${item.reasons.map(reason => `<li>${escapeHtml(reason)}</li>`).join("")}
      </ul>
      <button type="button" class="primary recommendation-add" data-add-recommendation="${escapeHtml(item.id)}">
        Agregar pick ${formatAmerican(item.american)}
      </button>
    </article>`;
}

function buildSmartParlay(maxPicks = 3) {
  const recommendations = buildDailyRecommendations();
  const selected = [];
  const usedGames = new Set();

  for (const item of recommendations) {
    if (item.grade === "Evitar" || item.ev == null || item.ev < 0.01) continue;
    if (usedGames.has(item.game.id)) continue;
    selected.push(item);
    usedGames.add(item.game.id);
...
function addSmartParlay() {
  const picks = buildSmartParlay(3);
  if (!picks.length) {
    showToast("No hay suficientes señales positivas para crear un parley.", "error");
    return;
  }
  ...
  showToast(`${added} picks agregados al parley inteligente.`);
}

function installSmartParlayButton() {
  const parleyHeading = document.querySelector("#parleySection .section-head, .parley-section .section-head");
  if (!parleyHeading || document.querySelector("#smartParlayBtn")) return;

  const button = document.createElement("button");
  button.id = "smartParlayBtn";
  button.type = "button";
  button.className = "text-btn smart-parlay-btn";
  button.textContent = "Crear parley inteligente";
  button.addEventListener("click", addSmartParlay);
  parleyHeading.appendChild(button);
}
...
function restoreNotice() {
  ...
  notice.textContent =
    "ParleyLab 5.0: recomendaciones, confianza heurística y EV estimado por mercado.";
